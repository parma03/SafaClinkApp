    ...
    if (mysqli_num_rows($checkquery) > 1) {
        ...
        if (!empty($_FILES['gambar']['name'])) {
            ...
            if (move_uploaded_file($_FILES["gambar"]["tmp_name"], $targetDir . $fileMenu)) {
                ...
                $query = "INSERT INTO tb_user (nama, email, nohp, username, password, role, profile) VALUES ('$nama', '$email','$nohp', '$username', '$password', 'Owner', '$gambar')";
            ...
            $query = "INSERT INTO tb_user (nama, email, nohp, username, password, role) VALUES ('$nama', '$email','$nohp', '$username', '$password', 'Owner')";
            $execute = mysqli_query($conn, $query);
            $check = mysqli_affected_rows($conn);
